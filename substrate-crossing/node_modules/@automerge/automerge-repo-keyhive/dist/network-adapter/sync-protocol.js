...
export class SyncProtocol {
    ...
    requestKeyhiveSync() {
        ...
        void this.initiateKeyhiveSync(peerId, false, false)
            .catch((error) => log.error("[AMRepoKeyhive] Periodic sync failed:", error))
            .finally(() => {
    ...
    async sendKeyhiveSyncOps(message, metrics) {
        ...
        await this.keyhiveQueue.run(async () => {
            ...
            if (syncResponderTotal !== undefined &&
    ...
    async handleKeyhiveSyncCheck(message, metrics) {
        ...
        await this.keyhiveQueue.run(async () => {
            ...
            const ourSyncpointMatchesTheirTotal = peer.syncpoint !== null && peer.syncpoint === theirTotalForUs;
            const theirSyncpointMatchesOurTotal = theirSyncpoint === ourTotalForThem;
            const digestsMatch = arraysEqual(ourDigest, theirDigest);
            if (ourSyncpointMatchesTheirTotal &&
        ...
        try {
            ...
            if (events.length > this.archiveThreshold) {
                ...
                void this.keyhiveStorage
                    ...
                    .catch((error) => log.error("[AMRepoKeyhive] Failed to save archive after large batch:", error));
            }
            else {
    ...
    async getHashesForPeerPair(peerA, peerB, metrics) {
        ...
        const hashesForA = (await this.cache.getHashesForPeer(this.keyhive, peerA, metrics)) ??
            new Map();
        const hashesForB = (await this.cache.getHashesForPeer(this.keyhive, peerB, metrics)) ??
            new Map();
