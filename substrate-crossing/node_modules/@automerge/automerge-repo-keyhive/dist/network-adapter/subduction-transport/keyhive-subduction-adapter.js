...
import { ContactCard, Signed } from "@keyhive/keyhive/slim";
...
import { decodeSubductionKeyhiveMessage, decodeSignedMessage, encodeSubductionKeyhiveMessage, encodeSignedMessage, peerIdToSubduction, } from "./codec.js";
...
const KEYHIVE_MESSAGE_TYPES = new Set([
 ...
 */
export class KeyhiveSubductionAdapter extends EventEmitter {
    localPeerId;
    remotePeerId;
    subductionReady;
    keyhive;
    ...
    constructor(options) {
        ...
        this.subductionReady = Promise.resolve(options.subduction).then(async (subduction) => {
        ...
        if (cachingMode === "periodic") {
            ...
            this.periodicCacheRefreshId = setInterval(() => {
                void this.keyhiveQueue
                    ...
                    .catch((err) => log.error("[KeyhiveSubductionAdapter] PeriodicEventCache refresh failed:", err));
            }, syncRequestInterval);
            void this.keyhiveQueue
                .run(() => periodicCache.refresh(this.keyhive))
                .catch((err) => log.error("[KeyhiveSubductionAdapter] Initial PeriodicEventCache refresh failed:", err));
        }
        else {
        ...
        void this.subductionReady
            .then(({ subduction }) => subduction.registerFrameHandler({
            onMessage: (payload, _peerId) => {
                void this.handleInbound(payload).catch((err) => log.error("[KeyhiveSubductionAdapter] inbound SUK handler failed:", err));
            },
            onPeerDisconnect: (_peerId) => {
                this._connected = false;
                this.syncProtocol.onPeerDisconnected(this.remotePeerId);
                this.emit("peer-disconnected", { peerId: this.remotePeerId });
            },
        }))
            .catch((err) => log.error("[KeyhiveSubductionAdapter] failed to register frame handler:", err));
     ...
     */
    syncKeyhive(includeContactCard = false) {
    ...
    disconnect() {
        ...
        void this.subductionReady
            ...
            .catch((err) => log.error("[KeyhiveSubductionAdapter] failed to clear frame handler:", err));
    }
    send(message, contactCard) {
        if (!message.type || !KEYHIVE_MESSAGE_TYPES.has(message.type)) {
            log.warn(`[KeyhiveSubductionAdapter] dropping non-keyhive message type=${message.type}`);
            return;
        }
        const seqNumber = this.pending.register();
        void (async () => {
            try {
                const bytes = await this.keyhiveQueue.run(() => this.signAndEncode(message, contactCard));
                this.pending.fire(seqNumber, () => {
                    void this.subductionReady
                        .then(({ subduction, wasmPeerId }) => subduction.sendKeyhiveMessage(bytes, wasmPeerId))
                        .catch((err) => log.warn("[KeyhiveSubductionAdapter] sendKeyhiveMessage failed:", err));
                });
            }
            catch (err) {
                log.error(`[KeyhiveSubductionAdapter] failed to sign+frame (type=${message.type}):`, err);
                this.pending.cancel(seqNumber);
            }
        })();
    }
    async signAndEncode(message, contactCard) {
        const inlineDataCbor = "data" in message && message.data instanceof Uint8Array
            ? message.data
            : new Uint8Array();
    ...
    async handleInbound(payload) {
        ...
        try {
            ...
            log.error("[KeyhiveSubductionAdapter] Rust KeyhiveMessage decode failed:", err);
            return;
        }
        const verifyingKeyPeer = peerIdFromVerifyingKey(signed.verifyingKey);
        if (decoded.senderId !== verifyingKeyPeer) {
            log.error("[KeyhiveSubductionAdapter] sender mismatch: payload says", decoded.senderId, "verifyingKey says", verifyingKeyPeer);
            return;
        }
        ...
        try {
            ...
            log.error(`[KeyhiveSubductionAdapter] dispatchByType failed (type=${decoded.type}):`, err);
        }
    }
}
async function waitForPeer(subduction, remotePeerBytes) {
    for (;;) {
        const peers = await subduction.getConnectedPeerIds();
        for (const p of peers) {
            const bytes = p.toBytes();
            if (bytes.length === remotePeerBytes.length &&
