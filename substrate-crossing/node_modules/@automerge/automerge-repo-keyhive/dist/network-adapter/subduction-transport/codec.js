 ...
 */
export function isSukFrame(bytes) {
    if (bytes.length < 4)
        return false;
    return (bytes[0] === SUK_SCHEMA[0] &&
 ...
 */
export function encodeSukFrame(payload) {
 ...
 */
export function decodeSukFrame(bytes) {
    if (!isSukFrame(bytes)) {
        throw new SukFrameError(`bad schema header: expected SUK\\0 got ${[...bytes.slice(0, 4)]
    ...
    const totalSize = ((bytes[4] << 24) | (bytes[5] << 16) | (bytes[6] << 8) | bytes[7]) >>> 0;
    if (totalSize !== bytes.length) {
        throw new SukFrameError(`size mismatch: declared ${totalSize}, actual ${bytes.length}`);
    }
    return bytes.slice(8);
}
export function encodeSignedMessage(msg) {
    return encode(msg);
}
export function decodeSignedMessage(bytes) {
    const obj = decode(bytes);
    if (typeof obj?.contactCard !== "string") {
        throw new SukFrameError(`decoded SignedMessage missing string contactCard field`);
    }
 ...
 */
export function peerIdToSubduction(peerId) {
    const dashIdx = peerId.indexOf("-");
    const b64 = dashIdx === -1 ? peerId : peerId.slice(0, dashIdx);
    const binStr = atob(b64);
    if (binStr.length !== 32) {
        throw new SukFrameError(`peerId verifying-key length: expected 32, got ${binStr.length}`);
    }
    const verifying_key = new Uint8Array(32);
    for (let i = 0; i < 32; i++)
        verifying_key[i] = binStr.charCodeAt(i);
 ...
 */
export function peerIdFromSubduction(rust) {
    if (rust.verifying_key.length !== 32) {
        throw new SukFrameError(`Rust peer-id verifying_key length: expected 32, got ${rust.verifying_key.length}`);
    }
    let bin = "";
    for (let i = 0; i < 32; i++)
...
const TYPE_BY_RUST_VARIANT = Object.fromEntries(Object.entries(RUST_VARIANT_BY_TYPE).map(([k, v]) => [
    v,
    k,
]));
 ...
 */
export function encodeSubductionKeyhiveMessage(input) {
 ...
 */
export function decodeSubductionKeyhiveMessage(payload) {
    const obj = decode(payload);
    const variants = Object.keys(obj);
    if (variants.length !== 1) {
        throw new SukFrameError(`KeyhiveMessage envelope has ${variants.length} variants (expected 1)`);
    }
    ...
    const senderId = peerIdFromSubduction(v.sender_id);
    const targetId = peerIdFromSubduction(v.target_id);
...
// ---------------------------------------------------------------------------
function decodeInlineForType(type, inlineDataCbor) {
    if (type === "keyhive-sync-request-contact-card" ||
        type === "keyhive-sync-missing-contact-card") {
        return {};
    }
    const obj = (inlineDataCbor.length === 0 ? {} : decode(inlineDataCbor));
    switch (type) {
...
function encodeInlineForType(type, v) {
    let inline;
    switch (type) {
        case "keyhive-sync-request":
        ...
        case "keyhive-sync-check":
            inline = {
                ...
                senderDigest: v.sender_digest ?? ZERO_DIGEST,
            };
            break;
        case "keyhive-sync-confirmation":
