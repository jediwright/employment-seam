...
export function peerIdFromSigner(signer, suffix = "") {
    return peerIdFromVerifyingKey(signer.verifyingKey, suffix);
}
export function keyhiveIdentifierFromPeerId(peerId) {
...
export function verifyingKeyPeerIdWithoutSuffix(peerId) {
...
export function unwrapWasmError(error) {
    if (error &&
        typeof error === "object" &&
        "toError" in error &&
        typeof error.toError === "function") {
...
// unprotected as well.
export function isUnprotectedDocId(bytes) {
    if (bytes.length < 32)
        return true;
    for (let i = 16; i < 32; i++) {
        if (bytes[i] !== 0)
 ...
 */
export function isUnprotectedDoc(url) {
...
export class UnprotectedDocError extends Error {
    constructor(operation, url) {
        super(`${operation}: ${url} is an unprotected (pre-keyhive) document with no keyhive state`);
...
export async function getEventsForAgent(keyhive, agent) {
...
export async function getEventHashesForAgent(keyhive, agent) {
    const hashMap = new Map();
    // Get relevant membership + prekey + CGKA hashes for the agent
    const eventHashes = await keyhive.eventHashesForAgent(agent);
    for (const hash of eventHashes) {
        hashMap.set(hash.toString(), hash);
    }
    // Get the agent's own prekey hashes
    const keyOpHashes = await agent.keyOpHashes();
