...
import { Archive, CiphertextStore, DocumentId as KeyhiveDocumentId, Keyhive, Signer, } from "@keyhive/keyhive/slim";
...
import { LegacyAutomergeRepoKeyhive, AutomergeRepoKeyhive, generateDoc, keyhiveIdFactory, } from "./automerge-repo-keyhive.js";
 ...
 */
export function docIdFromAutomergeUrl(url) {
...
function resolveSyncServer(selection) {
    if (selection === "none")
        return null;
...
async function bootstrapKeyhive(options) {
    ...
    const uniqueIdHash = new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(peerIdSuffix)));
    const keyhiveStorage = new KeyhiveStorage(uniqueIdHash, options.storage);
    const keyhive = await keyhiveStorage.loadOrCreateKeyhive(signer, uniqueIdHash, emitter.handleKeyhiveEvent);
    ...
    const syncServer = serverIdentity
        ? await syncServerFromContactCard(serverIdentity.contactCardJson, serverIdentity.peerId, keyhive, keyhiveStorage)
        : null;
    return {
        active,
        keyhive,
 ...
 */
function setupEventFlushListener(keyhiveContext, driver, options) {
    const { emitter, keyhiveQueue, keyhiveStorage, keyhive } = keyhiveContext;
    let syncTimeout;
    let pendingEventBytes = [];
        ...
        if (!flushQueued) {
            ...
            void keyhiveQueue
                ...
                if (eventsToSave.length > 0) {
                    log.debug(`[AMRepoKeyhive] Keyhive updated. Saving ${eventsToSave.length} events.`);
                ...
                .catch((error) => log.error("[AMRepoKeyhive] Event flush failed:", error));
        }
    };
...
async function buildLegacyHive(options) {
    const { automaticArchiveIngestion = true, onlyShareWithSyncServer = false, periodicallyRequestSync = true, cachingMode = "none", syncRequestInterval = 2000, batchInterval, retryPendingFromStorage = true, enableCompaction = true, archiveThreshold = 200, } = options;
    ...
    const createKeyhiveNetworkAdapter = (networkAdapter, wrapOptions = {}) => {
        const { onlyShareWithSyncServer: onlyServer = false, periodicallyRequestSync: periodicSync = false, syncRequestInterval: interval = 2000, batchInterval: batchIntervalOverride, archiveThreshold: archiveThresholdOverride, } = wrapOptions;
        if (onlyServer && !serverPeerIdHardcoded) {
            throw new Error('onlyShareWithSyncServer requires a configured sync server (syncServer is "none")');
        }
    ...
    const cleanupEventFlushListener = setupEventFlushListener(bootstrap, keyhiveNetworkAdapter, { automaticArchiveIngestion });
    await keyhiveStorage.savePrekeySecrets(keyhive);
    const hive = new LegacyAutomergeRepoKeyhive(active, keyhive, keyhiveStorage, peerId, syncServer, keyhiveNetworkAdapter, emitter, keyhiveIdFactory(keyhiveNetworkAdapter, keyhive), createKeyhiveNetworkAdapter);
...
async function buildHive(options) {
    const { automaticArchiveIngestion = true, cachingMode = "periodic", syncRequestInterval = 2000, periodicallyRequestSync = true, } = options;
    const keyhiveContext = await bootstrapKeyhive(options);
    const { active, keyhive, keyhiveStorage, peerId, emitter, keyhiveQueue, serverPeerIdHardcoded, } = keyhiveContext;
    const remotePeerId = options.remotePeerId ?? serverPeerIdHardcoded;
    if (!remotePeerId) {
        throw new Error('initializeAutomergeRepoKeyhive: the subduction path syncs against a single remote, so syncServer "none" requires an explicit remotePeerId');
    }
    ...
    const cleanupEventFlushListener = setupEventFlushListener(keyhiveContext, networkAdapter, { automaticArchiveIngestion });
    await keyhiveStorage.savePrekeySecrets(keyhive);
    networkAdapter.syncKeyhive(true);
    const createKeyhiveNetworkAdapter = (legacyAdapter, wrapOptions = {}) => {
        const { onlyShareWithSyncServer: onlyServer = false, periodicallyRequestSync: periodicSync = false, syncRequestInterval: interval = 2000, batchInterval: batchIntervalOverride, archiveThreshold: archiveThresholdOverride, } = wrapOptions;
    ...
    const hive = new AutomergeRepoKeyhive(active, keyhive, keyhiveStorage, peerId, emitter, networkAdapter, idFactory, createKeyhiveNetworkAdapter, blobInterceptor);
    hive.registerCleanup(cleanupEventFlushListener);
    return hive;
}
/**
 ...
 */
export async function initializeLegacyAutomergeRepoKeyhive(options) {
    initKeyhiveWasm();
    const { createRepo, repo: repoOptions, shareConfigDebounceMs, onBeforeShareConfigChanged, ...hiveOptions } = options;
...
export async function initializeAutomergeRepoKeyhive(options) {
    ...
    let resolveSubduction;
    const subductionPromise = new Promise((resolve) => {
    ...
    void repo.subduction
        .then(resolveSubduction)
        .catch((err) => log.error("[AMRepoKeyhive] Failed to resolve repo.subduction:", err));
    hive.linkRepo(repo, {
        debounceMs: shareConfigDebounceMs,
        onBeforeShareConfigChanged,
    });
...
export async function receiveContactCard(keyhive, contactCard, keyhiveStorage) {
...
export async function getPendingOpHashes(keyhive) {
    const pendingOps = await keyhive.pendingEventHashes();
    return pendingOps ? Array.from(pendingOps.keys()) : [];
}
async function loadOrCreateKeyPairAndSigner(storage, keyPair) {
    if (keyPair) {
...
export class KeyhiveStorage {
    ...
    isEventWriteSuppressed() {
        return this.suppressEventWrites;
    }
    async withSuppressedEventWrites(fn) {
    ...
    constructor(keyhiveStorageId, storage) {
        this.keyhiveStorageId = keyhiveStorageId;
        this.storage = storage;
    }
    async removeNonPendingEvents(eventChunks, pendingKeys) {
        for (const chunk of eventChunks) {
            const isPending = pendingKeys.some((pk) => pk.length === chunk.key.length &&
    ...
    async saveKeyhiveWithHash(kh) {
        ...
        await this.storage.save([KEYHIVE_DB_KEY, KEYHIVE_ARCHIVES_KEY, hash], khBytes);
    }
    async saveEventWithHash(event) {
        const eventBytes = event.toBytes();
        await this.saveEventBytesWithHash(eventBytes);
    }
    async saveEventBytesWithHash(eventBytes) {
    ...
    async saveLeafSecret(secretBytes) {
        const canonical = new Uint8Array(secretBytes);
        const hash = await crypto.subtle.digest("SHA-256", canonical);
        const hashHex = uint8ArrayToHex(new Uint8Array(hash));
        await this.storage.save([KEYHIVE_DB_KEY, KEYHIVE_LEAF_SECRETS_KEY, hashHex], canonical);
        return hashHex;
    }
    async savePrekeySecrets(kh) {
    ...
    async compact(kh) {
        ...
        log.debug(`[AMRepoKeyhive] Compacting: ${keyhiveArchiveChunks.length} archives, ${keyhiveEventsChunks.length} events`);
        // Ingest all archives
        for (const chunk of keyhiveArchiveChunks) {
            if (chunk.data) {
                try {
                    ...
                    log.warn(`[AMRepoKeyhive] Failed to ingest archive during compaction:`, error);
                }
            }
        }
        ...
        // Ingest all events
        const eventsBytes = keyhiveEventsChunks
        ...
        if (eventsBytes.length > 0) {
            try {
                pendingKeys = (await kh.ingestEventsBytes(eventsBytes))
                    .map((bytes) => dataToKey.get(bytes.toString()))
                    .filter((key) => key !== undefined);
            }
            catch (error) {
                log.warn(`[AMRepoKeyhive] Failed to ingest events during compaction:`, error);
            }
        }
        ...
        log.debug(`[AMRepoKeyhive] Compaction complete. ${pendingKeys.length} pending events retained.`);
    }
    async ingestKeyhiveFromStorage(kh) {
        const keyhiveArchiveChunks = await this.storage.loadRange([
            KEYHIVE_DB_KEY,
            KEYHIVE_ARCHIVES_KEY,
        ]);
        ...
        for (const chunk of keyhiveArchiveChunks) {
            if (chunk.data) {
                log.debug(`[AMRepoKeyhive] Ingesting archive from storage. Hash: ${chunk.key[2]}`);
                try {
                    await kh.ingestArchive(new Archive(chunk.data));
                }
                catch (error) {
                    log.warn(`[AMRepoKeyhive] Failed to re-ingest archive during recovery:`, error);
                }
            }
        }
        // Ingest all events
        const eventsBytes = keyhiveEventsChunks
            ...
            .filter((data) => data !== undefined);
        if (eventsBytes.length > 0) {
            log.debug(`[AMRepoKeyhive] Ingesting ${eventsBytes.length} events from storage`);
            try {
                await kh.ingestEventsBytes(eventsBytes);
            }
            catch (error) {
                log.warn(`[AMRepoKeyhive] Failed to ingest events during recovery:`, error);
            }
        }
        log.debug("[AMRepoKeyhive] Reading from storage completed");
    }
    async loadOrCreateKeyhive(signer, uniqueIdHash, event_handler) {
        const keyhiveArchiveChunks = await this.storage.loadRange([
            KEYHIVE_DB_KEY,
            KEYHIVE_ARCHIVES_KEY,
        ]);
        ...
        if (keyhiveArchiveChunks.length > 0) {
            ...
            if (firstChunk.data) {
                ...
                try {
                    ...
                    log.info(`[AMRepoKeyhive] Ingesting archive from storage (1 of ${chunk_count}). Hash: ${firstChunk.key[2]}`);
                    ...
                    for (let idx = 1; idx < keyhiveArchiveChunks.length; idx++) {
                        ...
                        if (chunk.data) {
                            log.info(`[AMRepoKeyhive] Ingesting archive from storage (${idx + 1} of ${chunk_count}). Hash: ${chunk.key[2]}`);
                            await kh.ingestArchive(new Archive(chunk.data));
                        }
                    }
                    // Ingest individual events
                    log.info(`[AMRepoKeyhive] Ingesting ${eventsBytes.length} keyhive events from storage.`);
                    let pendingKeys = [];
                    if (eventsBytes.length > 0) {
                        pendingKeys = (await kh.ingestEventsBytes(eventsBytes))
                            .map((bytes) => dataToKey.get(bytes.toString()))
                    ...
                    log.error("[AMRepoKeyhive] Failed to load Keyhive archive:", unwrapWasmError(error));
                }
            }
        }
        ...
        if (eventsBytes.length > 0) {
            ...
            try {
                ...
                log.error(`[AMRepoKeyhive] Failed to ingest keyhive events from storage:`, unwrapWasmError(e));
            }
        }
        await this.saveKeyhiveWithHash(kh);
        return kh;
