...
import { ChangeId, DocumentId as KeyhiveDocumentId, Encrypted, symmetricEncrypt, symmetricDecrypt, } from "@keyhive/keyhive/slim";
...
export class KeyhiveBlobInterceptor {
    ...
    #delivered = new Set();
    constructor(keyhive, queue, onEncrypted, storage) {
    ...
    async loadPersistedPcsKeyHashes() {
        if (!this.#storage)
            return;
    ...
    async #importNewLeafSecrets() {
        ...
        try {
            ...
            for (const chunk of chunks) {
                if (!chunk.data)
                    continue;
                const hashKey = chunk.key[chunk.key.length - 1];
                if (this.#importedLeafSecrets.has(hashKey))
                    continue;
    ...
    #queuePersist() {
        if (!this.#storage || this.#persistQueued)
            return;
    ...
    get docIdsAwaitingPcsKey() {
        return [...this.#docsAwaitingPcsKey];
    }
    lastPcsKeyHashForDoc(documentId) {
        return this.#lastPcsKeyHash.get(documentId);
    }
    async transformOutgoing(documentId, commitId, parents, blob, loadBlob) {
        ...
        return this.#queue.run(async () => {
            ...
            if (!doc) {
                log.debug(`[KeyhiveBlobInterceptor] transformOutgoing: no keyhive document for ${documentId}; dropping outgoing blob (a later save retries)`);
                return null;
            }
            ...
            if (!pcsHash) {
                ...
                if (!pcsHash) {
                    ...
                    log.debug(`[KeyhiveBlobInterceptor] transformOutgoing: no derivable PCS key for ${documentId} (even after importing leaf secrets); dropping outgoing blob (a later save retries)`);
                    return null;
                }
            }
            ...
            for (const parentHex of parents) {
                ...
                if (idBytes.length !== COMMIT_ID_BYTES)
                    continue;
            ...
            const predsCipher = symmetricEncrypt(selfKey, concatBytes(entries), hexToBytes(commitId));
    ...
    async transformIncoming(documentId, commitId, blob, loadBlob) {
        const { binaryDocumentId, unprotected } = parseDocId(documentId);
        if (unprotected)
            return blob;
        return this.#queue.run(async () => {
    ...
    async #decryptAndWalk(doc, commitId, blob, loadBlob, out, seen) {
        const parsed = parseEnvelope(blob);
        if (!parsed) {
            log.debug(`[KeyhiveBlobInterceptor] unrecognized blob envelope${commitId ? ` for commit ${commitId}` : " during bulk document load"}; leaving pending`);
            return false;
        }
        ...
        if (cachedKey) {
            try {
                ...
                log.debug(`[KeyhiveBlobInterceptor] cached key failed for commit ${commitId}; retrying via CGKA`);
            }
        }
        let plaintext;
        let selfKey;
        ...
        for (const { idHex, key } of preds) {
            if (!this.#blobKeys.has(idHex))
                this.#blobKeys.set(idHex, key);
            if (seen.has(idHex) || !loadBlob)
                continue;
            seen.add(idHex);
    ...
    async #recoverParentKey(doc, parentHex, loadBlob) {
        let parentBlob;
        try {
    ...
    async #decryptViaCgka(doc, encrypted) {
        try {
            ...
            if (await this.#importNewLeafSecrets()) {
                try {
                    ...
                    log.debug("[KeyhiveBlobInterceptor] CGKA decrypt failed after importing new leaf secrets; leaving pending:", retryError);
                    return null;
                }
            }
            log.debug("[KeyhiveBlobInterceptor] CGKA decrypt failed (no new leaf secrets to import); leaving pending:", firstError);
            return null;
        }
    }
}
 ...
 * @internal Exported for tests.
 */
export function encodeOuterEnvelope(inner, predsCipher) {
    const out = new Uint8Array(5 + inner.length + predsCipher.length);
    out[0] = ENVELOPE_VERSION;
    new DataView(out.buffer, out.byteOffset, out.byteLength).setUint32(1, inner.length, true);
    out.set(inner, 5);
 ...
 * @internal Exported for tests.
 */
export function decodeOuterEnvelope(bytes) {
    if (bytes.length < 5 || bytes[0] !== ENVELOPE_VERSION)
        return null;
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const innerLen = view.getUint32(1, true);
    if (5 + innerLen > bytes.length)
        return null;
    return {
 ...
 * @internal Exported for tests.
 */
export function decodePreds(plain) {
    const out = [];
...
function parseDocId(documentId) {
    const { binaryDocumentId } = parseAutomergeUrl(`automerge:${documentId}`);
    return {
        binaryDocumentId,
