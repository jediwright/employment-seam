 ...
 * @experimental This API is experimental and may change in future versions.
 */
export type SubChangeFn<T> = (val: NonNullable<T> extends string ? MutableText : NonNullable<T>) => T | void;
/**
 ...
 */
type StepValue<TObj, TSegment> = NonNullable<TObj> extends infer O ? TSegment extends number | Pattern ? O extends readonly (infer E)[] ? E : unknown : TSegment extends CursorMarker ? O extends string ? string : unknown : TSegment extends string ? TSegment extends keyof O ? O[TSegment] : unknown : unknown : unknown;
/**
 ...
 */
type HopCanBeAbsent<TObj, TSegment> = NonNullable<TObj> extends infer O ? TSegment extends number | Pattern ? true : TSegment extends string ? TSegment extends keyof O ? undefined extends O[TSegment] ? true : false : false : false : false;
/** Leaf value type, traversing with intermediate nullability stripped. */
type ExactPathValue<TDoc, TPath extends readonly any[]> = TPath extends readonly [] ? TDoc : TPath extends readonly [infer First, ...infer Rest] ? ExactPathValue<StepValue<TDoc, First>, Rest> : unknown;
/**
 ...
 */
type PathIsNullable<TDoc, TPath extends readonly any[]> = undefined extends TDoc ? true : TPath extends readonly [infer First, ...infer Rest] ? HopCanBeAbsent<TDoc, First> extends true ? true : PathIsNullable<StepValue<TDoc, First>, Rest> : false;
/**
 ...
 * `undefined`-free.
 */
export type PathValue<TDoc, TPath extends readonly any[]> = PathIsNullable<TDoc, TPath> extends true ? ExactPathValue<TDoc, TPath> | undefined : ExactPathValue<TDoc, TPath>;
export type InferSubType<TDoc, TPath extends readonly any[]> = PathValue<TDoc, TPath>;
...
type StepValueFromString<TObj, TSegment> = NonNullable<TObj> extends infer O ? TSegment extends CursorRangeMarker ? O extends string ? string : unknown : TSegment extends number ? O extends readonly (infer E)[] ? E : unknown : TSegment extends string ? TSegment extends keyof O ? O[TSegment] : unknown : unknown : unknown;
/**
 ...
 */
type HopCanBeAbsentFromString<TObj, TSegment> = NonNullable<TObj> extends infer O ? TSegment extends number ? true : TSegment extends CursorRangeMarker ? false : TSegment extends string ? TSegment extends keyof O ? undefined extends O[TSegment] ? true : false : false : false : false;
/** Leaf value type for a parsed string path, intermediate nullability stripped. */
type ExactPathValueFromString<TDoc, TPath extends readonly any[]> = TPath extends readonly [] ? TDoc : TPath extends readonly [infer First, ...infer Rest] ? ExactPathValueFromString<StepValueFromString<TDoc, First>, Rest> : unknown;
/** Whether a parsed string path can resolve to `undefined`. */
type PathIsNullableFromString<TDoc, TPath extends readonly any[]> = undefined extends TDoc ? true : TPath extends readonly [infer First, ...infer Rest] ? HopCanBeAbsentFromString<TDoc, First> extends true ? true : PathIsNullableFromString<StepValueFromString<TDoc, First>, Rest> : false;
/** Recursively traverse document type using parsed path segments */
type PathValueFromString<TDoc, TPath extends readonly any[]> = PathIsNullableFromString<TDoc, TPath> extends true ? ExactPathValueFromString<TDoc, TPath> | undefined : ExactPathValueFromString<TDoc, TPath>;
/** Infer the sub-handle value type from a document type and path string */
export type InferSubTypeFromString<TDoc, P extends string> = PathValueFromString<TDoc, SegmentsFromString<P>>;
export {};
