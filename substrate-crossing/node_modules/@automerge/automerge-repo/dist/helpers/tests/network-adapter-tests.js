    ...
    describe(`Network adapter acceptance tests ${title ? `(${title})` : ""}`, () => {
        it("can sync 2 repos", async () => {
            const doTest = async (a, b) => {
                ...
                await Promise.all([aliceRepo, bobRepo].map(r => eventPromise(r.networkSubsystem, "peer")));
                ...
                assert.equal((await bobHandle).fullDoc()?.foo, "bar");
                ...
                assert.equal(aliceHandle.fullDoc().foo, "baz");
            };
        ...
        it("can sync 3 repos", async () => {
            ...
            assert.equal(bobHandle.fullDoc().foo, "bar");
            assert.equal(charlieHandle.fullDoc().foo, "bar");
            ...
            assert.equal(bobHandle.fullDoc().foo, "baz");
            assert.equal(charlieHandle.fullDoc().foo, "baz");
            teardown();
        });
 ...
 */
const waitForDocReceivedByAllPeers = async (repo, documentId) => {
    const docSync = repo.synchronizer.docSynchronizers[documentId];
    if (!docSync) {
        throw new Error(`waitForDocReceivedByAllPeers: no DocSynchronizer for ${documentId}`);
    }
...
//# sourceMappingURL=network-adapter-tests.js.map