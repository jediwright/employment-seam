...
**Status:** ACTIVE — entries appended through Run 8 (2026-08-31 UTC);
Item 2.2 note appended; Runs 6, 7, and 8 carry operator-notes sections. One entry block per crossing run; append-only; do not edit
...
### Run 8 — operator notes (2026-08-30)

```
toctou_observation_note: Item 3.3 — TOCTOU between mint and fire. Aggregated-data pattern is the strongest test of `authorizedContentDigest` binding; TOCTOU surface named. Two windows measured from the positive-leg crossing log: (w1) mint→re-verification, intent-record-written 00:34:58.826Z → fire-verification-pass 00:34:58.826Z, <1 ms; (w2) residual, fire-verification-pass 00:34:58.826Z → put-record-fired 00:34:58.826Z, <1 ms. The TOCTOU leg shows the check live: assembly document mutated after mint (test-hook injection, named deviation) → fire-verification-blocked at 00:34:58.335Z, 2 ms after intent-record-written 00:34:58.333Z; no put-record-fired event in that log; intent record document-resident (1), not retracted; crossing reads crossing-unconfirmed; retry requires a fresh gate pass (steps 1–3h). Bounding mechanism: single-process, single-run pipeline; no awaited I/O between the step-8v read and the fire stamp beyond the microtask boundary. Named honestly: the re-verification collapses the exploitable window from (mint→fire) to (8v-read→fire); it does not eliminate it — un-eliminable. Two surfaces implemented: A (assembly document mutated after mint → step 8v BLOCKS, gate-style, Q2 ruling) and B (outgoing payload content object diverged from the minted digest → fire wrapper THROWS before publish, seam-fault posture). Surface B evidence is unit-test-only by design (a live mismatched publish would mint garbage evidence) — scoped absence, operator-accepted; the positive leg exercised the Surface B check live on a matching payload and passed.
ac_confirmation:     AC-3.3.1, AC-3.3.2, AC-3.3.3, AC-3.3.4, AC-3.3.5, AC-3.3.6 confirmed by operator against this entry and this notes section, 2026-08-30. Adversarial step (spec r2): the positive leg is the post-block retest — the gate did not latch on the negative or TOCTOU blocks. Negative-leg block 00:34:58.317Z precedes the earliest intent-record timestamp in the run (00:34:58.333Z, TOCTOU leg) — AC-3.3.3 ordering clause met. Byte-append tamper changed length and hash; the gate blocked on hash inequality only (FM3).
f331_note:           F-3.3-1 — the Surface B check, on its first in-container run, caught the Item 1.2-era e2e fixture (test/crossing-fire.test.ts, timed publish + relay sim) wiring makeRecord(): a payload divergent from the content the intent authorized, masked until fire-time verification existed. Fixture-only: its publish is an in-process mock; no live record affected. Runs 1–7 published via the runner, whose record is built from the assembled content the seam hash-checked at step 3 — clean by runner discipline, now enforced by the seam. Remediation in this diff: fixture builds its record from the authorized content and asserts on the published title/content/createdAt. Sweep: the remaining makeRecord() uses call makeTimedPutRecord without an intent (timing-capture tests) — skip-by-design, correct as-is. Operator disposition: real finding, registered; no SL-0188 exposure (no finding against the 3h gate order or the optional-field rules).
relay_note:          F-3.2-5 — fixed in this diff: the Jetstream watcher opens immediately before the fireable leg (uniform rule; blocked legs never fire). Run 8: relay_ingested_at 00:34:59.410Z, 252 ms after PDS accept — the relay-path ~ carried on SL-0188 lifts for the aggregated scenario. Delayed-release behaviour verified by inspection only (Run 8 has no embargo wait); named lift event: any future delayed-release run validates it incidentally.
runner_wording_note: F-3.2-6 — fixed: the positive-leg "attempt at" (00:34:58.819Z) is captured pre-leg and precedes gate-check-started (00:34:58.819Z). The crossing log remains authoritative.
deviation_note:      Q4 ruling (spike-D discipline): __testOnlyBetweenMintAndFire is a test-only function parameter on initiateCrossing(), invoked between step 7 and step 8v; OMITTED on the production path (omitted-never-null — present-but-not-a-function is a seam fault); never a record field, cannot enter canonical JSON or any digest. Used by the TOCTOU leg and the Item 3.3 unit tests only.
sl0188_stake_note:   SL-0188 validation event (Item 3.3): no Run 8 finding against the 3h gate order or the optional-field rules; the 3h step, D-2/D-7 checks, and Runs 1–7 crossingIntentRef binding are untouched by this diff.
appview_observation: surfaced — whtwnd.com/localboundary.bsky.social/3mudnwhpxpn25 renders the aggregated content (title "PC#8 governed crossing — Run 8 — doc_a"; both assembled sections doc_a and doc_b displayed in fixed order); seamCrossingRef not rendered — dropped at the AppView, unchanged Runs 4–8 (KL-2; intact at PDS via getRecord). Displayed date = content createdAt (fixture stamp, 8:34:56 PM local); no horizon on this run. Observed 2026-08-30 local.
```
