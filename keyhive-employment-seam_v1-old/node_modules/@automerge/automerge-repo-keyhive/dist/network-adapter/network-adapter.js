...
export class KeyhiveNetworkAdapter extends NetworkAdapter {
    ...
    batchInterval;
    keyhiveMsgBatch;
    streamingMetrics = new Metrics();
    metricsIntervalId;
    syncProtocol;
    constructor(options) {
        super();
        const { networkAdapter, contactCard, keyhive, keyhiveStorage, keyhiveQueue, periodicallyRequestSync, cachingMode = "none", hardcodedRemoteId = null, syncRequestInterval, batchInterval, retryPendingFromStorage = true, enableCompaction = true, archiveThreshold = 200, } = options;
        ...
        if (cachingMode === "periodic") {
            ...
            this.periodicCacheRefreshId = setInterval(() => {
                void this.keyhiveQueue.run(() => periodicCache.refresh(this.keyhive)).catch((error) => log.error("[AMRepoKeyhive] PeriodicEventCache refresh failed:", error));
            }, syncRequestInterval);
            // Initial refresh
            void this.keyhiveQueue.run(() => periodicCache.refresh(this.keyhive)).catch((error) => log.error("[AMRepoKeyhive] Initial PeriodicEventCache refresh failed:", error));
        }
        else {
        ...
        if (periodicallyRequestSync) {
            this.syncIntervalId = setInterval(() => this.syncProtocol.requestKeyhiveSync(), syncRequestInterval);
            this.fullSyncIntervalId = setInterval(() => this.syncProtocol.requestFullKeyhiveSync(), 5 * 60 * 1000);
        }
        if (enableCompaction) {
            this.compactionIntervalId = setInterval(this.runCompaction.bind(this), 60000);
        }
        ...
        if (this.isBatching()) {
            this.batchProcessor = new BatchProcessor(this.batchInterval, this.keyhive, async (message, data, metrics) => {
    ...
    send(message, contactCard) {
        ...
        if (message.type?.startsWith("keyhive-")) {
            void this.signAndSend(message, contactCard).catch((error) => log.error(`[AMRepoKeyhive] Failed to sign and send (type=${message.type}):`, error));
        }
        else {
            this.networkAdapter.send(message);
        }
    }
    async signAndSend(message, contactCard) {
        if (this.peerId === undefined) {
            throw new Error("peerId must be defined!");
        }
        ...
        try {
            const signedData = await this.keyhiveQueue.run(() => signData(this.keyhive, data, contactCard));
            ...
            log.error(`[AMRepoKeyhive] asyncSignAndSend FAILED for seq=${seqNumber}, type=${message.type}:`, error);
    ...
    receiveMessage(message) {
        try {
            if (this.hardcodedRemoteId &&
                ...
                log.debug(`[AMRepoKeyhive] Unknown remote peer ${message.senderId}. Ignoring message!`);
                return;
            }
            if (message.type === "subduction-connection") {
                this.emit("message", message);
            ...
            if (maybeKeyhiveMessageData) {
                if (verifyData(message.senderId, maybeKeyhiveMessageData)) {
                    if (!message.type?.startsWith("keyhive-")) {
                        ...
                        this.streamingMetrics.recordMessage(message.type, message.senderId, maybeKeyhiveMessageData.signed.payload?.byteLength ?? 0);
                        ...
                        }).catch((error) => log.error(`[AMRepoKeyhive] Error handling message (type=${message.type}, from=${message.senderId}):`, error));
                    }
                }
                else {
                    log.error(`[AMRepoKeyhive] verifyData FAILED for type=${message.type} from=${message.senderId} doc=${message.documentId}`);
                }
            }
            else {
                this.emit("message", message);
