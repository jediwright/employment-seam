...
function digestHex(d) {
    return Array.from(d, (x) => x.toString(16).padStart(2, "0")).join("");
}
export class SyncProtocol {
    lastEmittedTotalOps = 0n;
    syncRequestQueued = false;
    fullSyncRequestQueued = false;
    keyhive;
    keyhiveStorage;
    keyhiveQueue;
    peers;
    ...
    minSyncRequestInterval;
    minSyncResponseInterval;
    ...
    async handleKeyhiveMessage(message, keyhiveMessageData, metrics) {
        if (keyhiveMessageData.contactCard) {
            const contactCard = keyhiveMessageData.contactCard;
            await this.keyhiveQueue.run(() => receiveContactCard(this.keyhive, contactCard, this.keyhiveStorage));
        }
        message.data = keyhiveMessageData.signed.payload;
        return this.dispatchByType(message, metrics);
    }
    async dispatchByType(message, metrics) {
    ...
    syncKeyhive(maybeSenderId = undefined, includeContactCard = false, attemptRecovery = false) {
        void this.initiateKeyhiveSync(maybeSenderId, includeContactCard, attemptRecovery).catch((error) => log.error("[AMRepoKeyhive] Sync initiation failed:", error));
    }
    requestKeyhiveSync() {
        const peerId = this.getPeerId();
        if (peerId === undefined) {
        ...
        void this.initiateKeyhiveSync(peerId, false, false).catch((error) => log.error("[AMRepoKeyhive] Periodic sync failed:", error)).finally(() => {
            this.syncRequestQueued = false;
        });
    }
    requestFullKeyhiveSync() {
        const peerId = this.getPeerId();
        if (peerId === undefined) {
            return;
        }
        ...
        void this.initiateKeyhiveSync(peerId, false, false, true).catch((error) => log.error("[AMRepoKeyhive] Full sync failed:", error)).finally(() => {
    ...
    async initiateKeyhiveSync(maybeSenderId, includeContactCard, attemptRecovery = false, forceFullRequest = false) {
        const peerId = this.requirePeerId();
        await this.keyhiveQueue.run(async () => {
            if (attemptRecovery) {
                log.debug("[AMRepoKeyhive] Preparing for keyhive sync. Reading from storage");
                try {
            ...
            for (const targetId of this.peers.keys()) {
                ...
                if (!targetAgent) {
                    ...
                    if (peer !== undefined && peer.syncpoint !== null && !forceFullRequest) {
                        ...
                        log.debug(`[AMRepoKeyhive] Sending keyhive sync check to ${targetId} from ${senderId}: senderTotal=${senderTotal}, senderSyncpoint=${peer.syncpoint}`);
                        ...
                        log.debug(`[AMRepoKeyhive] Sending keyhive sync request to ${targetId} from ${senderId} with ${opHashes.length} local operations and ${pendingOpHashes.length} pending operations.`);
                        this.sendMessage(message, maybeContactCard);
                    }
                }
                const peer = this.peers.get(targetId);
    ...
    // peer.
    async sendKeyhiveSyncResponse(message, metrics) {
        if (!("data" in message) || !message.data) {
            log.error("[AMRepoKeyhive] Expected data in keyhive-sync-request");
            return;
        }
        if (message.type !== "keyhive-sync-request") {
            log.error(`[AMRepoKeyhive] Expected keyhive-sync-request, but got ${message.type}`);
            return;
        }
        const peerId = this.requirePeerId();
        const requestData = decode(message.data);
        const peerFoundHashes = requestData.found || [];
        const peerPendingHashes = requestData.pending || [];
        log.debug(`[AMRepoKeyhive] Received keyhive sync request from ${message.senderId} with ${peerFoundHashes.length} found hashes, ${peerPendingHashes.length} pending hashes`);
        ...
        await this.keyhiveQueue.run(async () => {
            ...
            if (!senderAgent) {
                log.debug(`[AMRepoKeyhive] No agent found for ${message.senderId}, sending keyhive-sync-request-contact-card`);
                const response = {
                    type: "keyhive-sync-request-contact-card",
                    senderId: peerId,
                ...
                log.debug(`[AMRepoKeyhive] asyncSendKeyhiveSyncResponse: Found ${localHashes.size} total local operation hashes for ${message.senderId} and ${pendingOpHashes.length} total pending hashes`);
                // Build map to look up peer hashes by string
                const peerFoundByHashString = new Map();
                for (const hash of peerFoundHashes) {
                    peerFoundByHashString.set(hash.toString(), hash);
                }
                // Build sets for set operations
                const pendingHashStrings = new Set(pendingOpHashes.map((h) => h.toString()));
                const peerPendingHashStrings = new Set(peerPendingHashes.map((h) => h.toString()));
                ...
                const hashStringsToSend = localHashStrings.difference(peerFoundHashStrings.union(peerPendingHashStrings));
                // Determine which ops we need to request from the peer
                const hashStringsToRequest = peerFoundHashStrings.difference(localHashStrings.union(pendingHashStrings));
                const requested = Array.from(hashStringsToRequest)
                    .map((str) => peerFoundByHashString.get(str))
                    .filter((hash) => hash !== undefined);
                let foundResult = { events: [], cborEvents: [] };
                ...
                log.debug(`[AMRepoKeyhive] Sending keyhive sync response to ${message.senderId} from ${peerId}`);
    ...
    async sendKeyhiveSyncOps(message, metrics) {
        if (!("data" in message) || !message.data) {
            log.error("[AMRepoKeyhive] Expected data in keyhive-sync-response");
            return;
        }
        if (message.type !== "keyhive-sync-response") {
            log.error(`[AMRepoKeyhive] Expected keyhive-sync-response, but got ${message.type}`);
            return;
        }
        const peerId = this.requirePeerId();
        const responseData = decode(message.data);
        const requestedHashes = responseData.requested || [];
        const foundEvents = responseData.found || [];
        const syncResponderTotal = responseData.syncResponderTotal;
        const syncRequesterTotal = responseData.syncRequesterTotal;
        log.debug(`[AMRepoKeyhive] Received keyhive sync response from ${message.senderId}: ${foundEvents.length} ops found, ${requestedHashes.length} ops requested`);
        const queueEnterTime = Date.now();
        await this.keyhiveQueue.run(async () => {
            metrics.recordQueueWait(Date.now() - queueEnterTime);
            ...
            if (requestedHashes.length > 0) {
                const requestedHashStrings = new Set(requestedHashes.map((h) => h.toString()));
                ...
                if (requestedResult.events.length === 0) {
                    ...
                    if (requestedResult.events.length < requestedHashes.length) {
                        log.warn(`[AMRepoKeyhive] ${requestedHashes.length} keyhive events requested, ${requestedResult.events.length} found.`);
                    }
                    metrics.recordOpsSent(requestedResult.events.length);
                    log.debug(`[AMRepoKeyhive] Sending ${requestedResult.events.length} requested ops to ${message.senderId}`);
    ...
    async sendKeyhiveSyncMissingContactCard(message) {
        if (message.type !== "keyhive-sync-request-contact-card") {
            log.error(`[AMRepoKeyhive] Expected keyhive-sync-request-contact-card, but got ${message.type}`);
            return;
        }
        const peerId = this.requirePeerId();
        log.debug(`[AMRepoKeyhive] Sending keyhive-sync-missing-contact-card to ${message.senderId}`);
        const response = {
    ...
    async receiveKeyhiveSyncOps(message, metrics) {
        if (!("data" in message) || !message.data) {
            log.error("[AMRepoKeyhive] Expected data in keyhive-sync-ops");
            return;
        }
        if (message.type !== "keyhive-sync-ops") {
            log.error(`[AMRepoKeyhive] Expected keyhive-sync-ops, but got ${message.type}`);
            return;
        }
        const peerId = this.requirePeerId();
        ...
        let receivedEvents;
        let syncResponderTotal;
        let syncRequesterTotal;
        ...
        log.debug(`[AMRepoKeyhive] Received ${receivedEvents.length} keyhive events`);
        const queueEnterTime = Date.now();
        await this.keyhiveQueue.run(async () => {
            metrics.recordQueueWait(Date.now() - queueEnterTime);
    ...
    async handleKeyhiveSyncCheck(message, metrics) {
        if (!("data" in message) || !message.data) {
            log.error("[AMRepoKeyhive] Expected data in keyhive-sync-check");
            return;
        }
        const peerId = this.requirePeerId();
        const checkData = decode(message.data);
        const theirTotalForUs = checkData.senderTotal;
        const theirSyncpoint = checkData.senderSyncpoint;
        const theirDigest = checkData.senderDigest ?? new Uint8Array(32);
        ...
        await this.keyhiveQueue.run(async () => {
            ...
            if (!peer) {
                ...
                // sender.
                log.debug(`[AMRepoKeyhive] Auto-registering peer from sync-check: ${message.senderId}`);
                peer = new Peer();
            ...
            if (ourSyncpointMatchesTheirTotal && theirSyncpointMatchesOurTotal && digestsMatch) {
                log.debug(`[AMRepoKeyhive] Sync check PASS (short-circuit) for ${message.senderId}: ` +
            ...
            log.debug(`[AMRepoKeyhive] Sync check FALLBACK (full-sync) for ${message.senderId}: ` +
                `ourTotalForThem=${ourTotalForThem} theirTotalForUs=${theirTotalForUs} ` +
                `ourSyncpoint=${peer.syncpoint ?? "null"} theirSyncpoint=${theirSyncpoint} ` +
    ...
    async handleKeyhiveSyncConfirmation(message, metrics) {
        if (!("data" in message) || !message.data) {
            log.error("[AMRepoKeyhive] Expected data in keyhive-sync-confirmation");
            return;
        }
        const confirmData = decode(message.data);
        const confirmerTotal = confirmData.confirmerTotal;
        metrics.recordSyncConfirmationReceived();
        const peer = this.peers.get(message.senderId);
        if (peer) {
            peer.syncpoint = confirmerTotal;
            log.debug(`[AMRepoKeyhive] Updated syncpoint for ${message.senderId}: ${confirmerTotal}`);
        }
    }
    ...
    async ingestAndRetry(events, senderId, metrics) {
        log.debug(`[AMRepoKeyhive] Ingesting ${events.length} keyhive events from ${senderId}`);
        try {
            let pendingEvents = null;
            try {
                pendingEvents = await this.keyhiveStorage.withSuppressedEventWrites(() => this.keyhive.ingestEventsBytes(events));
            }
            catch (error) {
            ...
            if (pendingEvents) {
                ...
                log.debug(`[AMRepoKeyhive] After ingestion: ${pendingEvents.length} pending events`);
            }
            ...
            if (!pendingEvents || pendingEvents.length > 0) {
                if (pendingEvents) {
                    log.warn(`[AMRepoKeyhive] ${pendingEvents.length} events stuck in pending${this.retryPendingFromStorage ? ". Reading from storage" : ""}`);
                }
                if (this.retryPendingFromStorage) {
                    ...
                    try {
                        ...
                        const retryPending = await this.keyhiveStorage.withSuppressedEventWrites(() => this.keyhive.ingestEventsBytes(events));
                        if (retryPending.length === 0) {
                            log.debug(`[AMRepoKeyhive] Successfully ingested all events after reading from storage`);
                        }
                        else {
                            log.warn(`[AMRepoKeyhive] Still have ${retryPending.length} pending events after reading from storage`);
                        }
                    }
                    catch (storageError) {
                        log.error(`[AMRepoKeyhive] Failed while reading from storage:`, storageError);
                    }
                }
            }
            // For large batches, write the full archive instead of individual events.
            if (events.length > this.archiveThreshold) {
                log.debug(`[AMRepoKeyhive] Large batch (${events.length} > ${this.archiveThreshold}): saving full archive instead of individual events`);
                void this.keyhiveStorage.saveKeyhiveWithHash(this.keyhive).catch((error) => log.error("[AMRepoKeyhive] Failed to save archive after large batch:", error));
            }
            else {
                void this.saveReceivedEvents(events).catch((error) => log.error("[AMRepoKeyhive] Failed to save received events:", error));
            }
            this.cache.onKeyhiveChanged();
    ...
    async saveReceivedEvents(events) {
    ...
    async getHashesForPeerPair(peerA, peerB, metrics) {
        ...
        const hashesForA = await this.cache.getHashesForPeer(this.keyhive, peerA, metrics) ?? new Map();
        const hashesForB = await this.cache.getHashesForPeer(this.keyhive, peerB, metrics) ?? new Map();
