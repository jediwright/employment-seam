...
import { Access, ChangeId, ContactCard, DocumentId as KeyhiveDocumentId, Identifier, } from "@keyhive/keyhive/slim";
import { MemorySigner } from "@automerge/automerge-subduction/slim";
import { docIdFromAutomergeUrl, receiveContactCard } from "./keyhive.js";
 ...
 */
export class AutomergeRepoKeyhiveBase {
    active;
    keyhive;
    keyhiveStorage;
    emitter;
     ...
     * path's `linkRepo`.
     */
    shareConfigTimer = null;
    constructor(active, keyhive, keyhiveStorage, emitter) {
        this.active = active;
     ...
     */
    noteLocalMembershipChange(_docUrl) { }
    /**
     ...
     */
    async constructSubductionSigner() {
        const jwk = await crypto.subtle.exportKey("jwk", this.active.keyPair.privateKey);
        if (!jwk.d) {
            throw new Error("[AMRepoKeyhive] constructSubductionSigner: key pair has no private scalar (non-extractable?)");
        }
    ...
    async receiveContactCard(contactCard) {
    ...
    async getDocForOperation(operation, docUrl) {
        const docId = docIdFromAutomergeUrl(docUrl);
        const doc = await this.keyhive.getDocument(docId);
        if (!doc) {
            throw new Error(`${operation}: document not found in keyhive for ${docUrl} (has it synced yet?)`);
        }
        return doc;
    }
    /**
     ...
     */
    async addMemberToDoc(docUrl, contactCard, access) {
        ...
        if (!agent) {
            throw new Error(`addMemberToDoc: contact card did not resolve to a keyhive agent (id ${uint8ArrayToHex(contactCard.id.toBytes())})`);
        }
     ...
     */
    async revokeMemberFromDoc(docUrl, member) {
        if (isUnprotectedDoc(docUrl))
            throw new UnprotectedDocError("revokeMemberFromDoc", docUrl);
        const identifier = typeof member === "string" ? new Identifier(hexToUint8Array(member)) : member;
        const agent = await this.keyhive.getAgent(identifier);
        if (!agent) {
            throw new Error(`revokeMemberFromDoc: member not found in keyhive (id ${uint8ArrayToHex(identifier.toBytes())})`);
        }
     ...
     * keyhive.
     */
    async setPublicAccess(docUrl, access) {
        if (isUnprotectedDoc(docUrl))
    ...
    async accessForDoc(id, docUrl) {
        const docId = this.toDocId(docUrl);
        if (isUnprotectedDocId(docId.toBytes()))
            return undefined;
     ...
     */
    async bestAccessForDoc(id, docUrl) {
        if (isUnprotectedDoc(docUrl))
            return undefined;
    ...
    async docMemberCapabilities(docUrl) {
        const docId = this.toDocId(docUrl);
        if (isUnprotectedDocId(docId.toBytes()))
            return [];
    ...
    /** @deprecated */
    async addSyncServerPullToDoc(docUrl) {
...
export class LegacyAutomergeRepoKeyhive extends AutomergeRepoKeyhiveBase {
    ...
    #linked = false;
    constructor(active, keyhive, keyhiveStorage, peerId, 
    /** Null when initialized with syncServer "none". */
    syncServer, networkAdapter, emitter, idFactory, createKeyhiveNetworkAdapter) {
        super(active, keyhive, keyhiveStorage, emitter);
        this.peerId = peerId;
        this.syncServer = syncServer;
        this.networkAdapter = networkAdapter;
        this.idFactory = idFactory;
        this.createKeyhiveNetworkAdapter = createKeyhiveNetworkAdapter;
    }
    /**
    ...
    buildServerSubductionPolicy() {
        const keyhive = this.keyhive;
        const hasAccess = async (id, docId, minAccess) => {
            try {
     ...
     */
    async addSyncServerRelayToDoc(docUrl) {
        if (isUnprotectedDoc(docUrl))
            throw new UnprotectedDocError("addSyncServerRelayToDoc", docUrl);
        if (!this.syncServer) {
            throw new Error('addSyncServerRelayToDoc: no sync server configured (syncServer is "none")');
        }
        const serverContactCard = ContactCard.fromJson(this.syncServer.contactCard.toJson());
...
;
export async function generateDoc(kh) {
    ...
    log.debug(`[AMRepoKeyhive] Generated Keyhive document with id ${doc.doc_id.toBytes()}`);
    return doc;
}
export function keyhiveIdFactory(_keyhiveNetworkAdapter, keyhive) {
    return async (_heads) => {
...
export class AutomergeRepoKeyhive extends AutomergeRepoKeyhiveBase {
    ...
    #docsWithLocalMembershipChanges = new Set();
    constructor(active, keyhive, keyhiveStorage, peerId, emitter, networkAdapter, idFactory, createKeyhiveNetworkAdapter, blobInterceptor) {
        super(active, keyhive, keyhiveStorage, emitter);
        this.peerId = peerId;
        this.networkAdapter = networkAdapter;
        this.idFactory = idFactory;
    ...
    noteLocalMembershipChange(docUrl) {
     ...
     */
    async #checkForMembershipNudges() {
        ...
        for (const documentId of docIds) {
            try {
                const doc = await this.keyhive.getDocument(docIdFromAutomergeUrl(`automerge:${documentId}`));
                if (!doc) {
                ...
                const baseline = this.#seenMembers.get(documentId) ??
                    new Set(members
                ...
                for (const c of members) {
                    ...
                    if (idHex === ourIdHex)
                        continue; // never nudge for our own membership
                    if (baseline.has(idHex))
                        continue; // not new
                    if (uint8ArrayToHex(c.proof.verifyingKey) === ourIdHex)
                        addedByUs++;
                }
                ...
                log.error(`[AMRepoKeyhiveSubduction] membership nudge check failed for ${documentId}:`, e);
            }
        }
    }
    /**
    ...
    async addSyncServerRelayToDoc(docUrl) {
        ...
        const agent = await this.keyhive.getAgent(identifier);
        if (!agent) {
            throw new Error(`addSyncServerRelayToDoc: sync server agent not yet known; retry after sync. remotePeerId=${this.networkAdapter.remotePeerId}`);
        }
        const docId = docIdFromAutomergeUrl(docUrl);
        const doc = await this.keyhive.getDocument(docId);
        if (!doc) {
            throw new Error(`addSyncServerRelayToDoc: document not found in keyhive for ${docUrl} (has it synced yet?)`);
        }
        await this.keyhive.addMember(agent, doc.toMembered(), Access.relay(), []);
        this.noteLocalMembershipChange(docUrl);
    }
    syncServerIdentifierHex() {
        return uint8ArrayToHex(keyhiveIdentifierFromPeerId(this.networkAdapter.remotePeerId).toBytes());
    }
}
