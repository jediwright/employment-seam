...
function resolveSyncServer(selection) {
    if (selection === undefined) {
        log.warn("[AMRepoKeyhive] No syncServer selection was passed; defaulting to the " +
 ...
 */
function setupEventFlushListener(bootstrap, driver, options) {
...
async function buildLegacyHive(options) {
    ...
    await keyhiveStorage.savePrekeySecrets(keyhive);
    return new LegacyAutomergeRepoKeyhive(active, keyhive, keyhiveStorage, peerId, syncServer, keyhiveNetworkAdapter, emitter, keyhiveIdFactory(keyhiveNetworkAdapter, keyhive), createKeyhiveNetworkAdapter);
}
/**
...
async function buildHive(options) {
    ...
    return new AutomergeRepoKeyhive(active, keyhive, keyhiveStorage, peerId, emitter, networkAdapter, idFactory, createKeyhiveNetworkAdapter, blobInterceptor);
}
/**
...
export class KeyhiveStorage {
    ...
    async removeNonPendingEvents(eventChunks, pendingKeys) {
        for (const chunk of eventChunks) {
            const isPending = pendingKeys.some((pk) => pk.length === chunk.key.length && pk.every((v, i) => v === chunk.key[i]));
            if (!isPending) {
