import { pause } from "../../src/helpers/pause.js";
import { NetworkAdapter } from "../../src/index.js";
export class DummyNetworkAdapter extends NetworkAdapter {
    #sendMessage;
    #connected = false;
    #ready = false;
    #readyResolver;
    #readyPromise = new Promise(resolve => {
        this.#readyResolver = resolve;
    });
    isReady() {
        return this.#ready;
    }
    whenReady() {
        return this.#readyPromise;
    }
    #forceReady() {
        if (!this.#ready) {
            this.#ready = true;
            this.#readyResolver?.();
        }
    }
    // A public wrapper for use in tests!
    forceReady() {
        this.#forceReady();
    }
    constructor(opts = { startReady: true }) {
        super();
        if (opts.startReady) {
            this.#forceReady();
        }
        this.#sendMessage = opts.sendMessage;
    }
    connect(peerId) {
        this.#connected = true;
        this.peerId = peerId;
    }
    disconnect() {
        this.#connected = false;
    }
    peerCandidate(peerId) {
        this.emit("peer-candidate", { peerId, peerMetadata: {} });
    }
    send(message) {
        if (!this.#connected) {
            return;
        }
        this.#sendMessage?.(message);
    }
    receive(message) {
        if (!this.#connected) {
            return;
        }
        this.emit("message", message);
    }
    static createConnectedPair({ latency = 0 } = {}) {
        // Default to microtask delivery. `setTimeout`-based delivery (any
        // `latency > 0`) is subject to event-loop starvation under concurrent
        // test load, which produces flaky round-trip-heavy tests. Callers that
        // actually want to simulate latency can still pass a positive value.
        const deliver = latency === 0
            ? (adapter, message) => Promise.resolve().then(() => adapter.receive(message))
            : (adapter, message) => pause(latency).then(() => adapter.receive(message));
        const adapter1 = new DummyNetworkAdapter({
            startReady: true,
            sendMessage: (message) => deliver(adapter2, message),
        });
        const adapter2 = new DummyNetworkAdapter({
            startReady: true,
            sendMessage: (message) => deliver(adapter1, message),
        });
        return [adapter1, adapter2];
    }
}
//# sourceMappingURL=DummyNetworkAdapter.js.map